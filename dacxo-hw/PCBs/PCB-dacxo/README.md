...
- current status of the audio buffer and clock rate adjustment (see description below)
...
- Digital supply as a raw 5V dc from an external small smps supply.
...
### Oscillator (reference klok) stability
Low jitter in the main audio reference klok, corresponds to low oscillator phase noise.
...
### Re-clocking of digital audio signals
...
the crystal reference clock and the clock inpout of this register.
This shall minimise the build-up of jitter on signals that pass through logic gates.

### Low-noise power supply on the digital ciruits
...
when the IC powersupply has small dips or peaks. That is an *enormous* variation in delay, as we aim for
...
This low-noise power is used for the crystal oscillators, the re-clock register, and the digital supply of the DAC chips.
In 'standby mode', this *3.3Vana* with its off-PCB transformer is switched off.

The other digital circuitry (notably the FPGA and the sp/dif receiver) are supplied from a separate 'digital' power.

### Good decoupling of IC power supply
To obtain a clean supply voltage and reduce EMC-induced noise, good power-supply decoupling is required
on the digital ICs. This is achieved by:

- a local decoupling capacitors very close to each chip.
...
Creating a good-sounding DAC from sp/dif inputs (optical or coax) poses a major extra challenge,
which can be avoided from an i2s or usb sound input:
Sp/dif audio is received with its own clock. Recovering that received clock inevitably leads to a
very bad (unstable, high jitter) clock signal.
Unfortunately, a fixed-frequency crystal oscillator might slightly deviate from the frequency of the received
audio data stream, because its frequency can typically deviate by +/- 100ppm (or 0.01%) from the nominal target.
...
stored in a cyclic buffer (a queue of samples). This buffer can be relatively small, lets say 64 samples.
...
If the buffer runs low (nearly empty), the clock divider to create the sample rate (word-)clock, is adjusted to be 1023.
Similarly, if the queue runs nearly full, the clock divider can be adjusted to 1025.
Effectively, this creates a (roughly) 0.1% deviation in the output clock rate.
Switching the clock frequency +/- 0.1% occurs at low rate: Suppose 48 samples in the queue need to be flushed or filled
at the modified frequency, that would take (48 / (samplerate x 0.1%)) is about 1 second.
Having the DAC speed-up or slow-down the output rate by 0.1% during about a second is truly in-audible.
This design choice can be considered a PLL, with a very low-frequency filter loop.
Obviously, the good properties are that a) a high-quality low-jitter clock can be used,
and b) all original audio samples are retained as is.

In this design, the sample buffer (the queue) as well as the clock dividers
for 1023 and 1025 (next to the default 1024) are all implemented in the on-board FPGA.
